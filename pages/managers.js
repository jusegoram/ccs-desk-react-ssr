    ...
    render() {
      const columns = [
        ...
        { Header: 'Company', accessor: 'company.name' },
        {
          ...
          id: 'terminatedAt',
          accessor: row => row.terminatedAt,
          Cell: ({ row }) => (row.terminatedAt ? moment(row.terminatedAt).format('MMMM Do at h:mm A') : null),
        },
      ]
      return (
        <Layout>
          <Query {...data.Employee.QUERY_managers} fetchPolicy="cache-and-network">
            {({ loading, data }) => {
              return (
                <Card>
